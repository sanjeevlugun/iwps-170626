...
namespace IWPS
{
    public partial class caa_list_wo_approve : Page
    {
        ...
        private void LoadWos()
        {
            try
            {
                ...
                string sql = @"SELECT w.WO_NO, w.ENQ_NO, w.LOI_NO, w.CONTRACT_VALUE, w.WO_DATE, w.CREATED_BY,
                               ...
                               WHERE w.STATUS = 'INITIATED' AND w.DEPT_CODE = :p1
        ...
        {
            if (e.CommandName != "ForwardWo" && e.CommandName != "RejectWo") return;
            ...
            {
                string cmdName = e.CommandName == "ForwardWo" ? "lbForward" : "lbReject";
                LinkButton lb = (LinkButton)r.FindControl(cmdName);
            ...
            string newStatus = e.CommandName == "ForwardWo" ? "FORWARDED" : "REJECTED";
            string action = e.CommandName == "ForwardWo" ? "FORWARDED TO HOD" : "REJECTED BY CAA";
            ...
            try
            {
                ...
                string msg = e.CommandName == "ForwardWo" ? "forwarded to HOD" : "rejected";
